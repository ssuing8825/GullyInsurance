...
namespace GullyInsurance.Policy.Domain
{
    ...
    public class AmendService
    {
        ...
        public void RunAmend(InsureVehicle policy)
        {
        ...
        private void AppendToStream(InsureVehicle policy)
        {
            using (IEventStream stream = store.OpenStream(policy.PolicyId, int.MinValue, int.MaxValue))
            {
        ...
        private static void OpenOrCreateStream(InsureVehicle policy)
        {
        ...
        private static void DispatchCommit(Commit commit)
        {
            ...
            try
            {
                foreach (EventMessage @event in commit.Events)
                    Console.WriteLine("Message dispatched " + ((InsureVehicle)@event.Body).PolicyId);
            }
            catch (Exception)
            {
